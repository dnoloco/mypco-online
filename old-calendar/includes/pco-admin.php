...
class pco_admin {
    ...
    private $model;
    private $webhook_handler; // NEW: For Google Forms integration
    private $signups_handler;    // ADD THIS
    private $stripe_handler;     // ADD THIS
    ...
    public function __construct(pco_api_model $model) {
        ...
        add_action('admin_init', [$this, 'handle_signup_form_submission']);
        add_action('admin_init', [$this, 'auto_fix_slashes_once']);
    ...
    /**
     * Handle signup form submission on admin_init (before any output)
     * This prevents "headers already sent" errors
     */
    public function handle_signup_form_submission() {
        // Only run on signup pages
        if (!isset($_GET['page']) || $_GET['page'] !== 'pco-signups') {
            return;
        }
        ...
        // Get signup ID if editing
        $signup_id = isset($_GET['id']) ? intval($_GET['id']) : null;
        ...
        $data = [
                ...
                'google_form_id' => sanitize_text_field(wp_unslash($_POST['google_form_id'])),
                'google_form_url' => esc_url_raw(wp_unslash($_POST['google_form_url'])),
                'max_attendees' => intval($_POST['max_attendees']),
                'payment_required' => isset($_POST['payment_required']) ? 1 : 0,
                'payment_amount' => floatval($_POST['payment_amount']),
                'payment_description' => sanitize_textarea_field(wp_unslash($_POST['payment_description'])),
                'allow_partial_payment' => isset($_POST['allow_partial_payment']) ? 1 : 0,
                'minimum_payment' => floatval($_POST['minimum_payment'] ?? 0),
                'is_active' => isset($_POST['is_active']) ? 1 : 0,
            ];

        // Save to database
        $saved_id = $this->signups_handler->save_signup($data);

        // Redirect - this happens BEFORE any WordPress output!
        wp_redirect(admin_url('admin.php?page=pco-signups&view=edit&id=' . $saved_id . '&updated=1'));
        exit;
    }

    public function auto_fix_slashes_once() {
        if (!isset($_GET['page']) || $_GET['page'] !== 'pco-signups') return;
        if (get_option('pco_slashes_fixed')) return;
        if (!current_user_can('manage_options')) return;

        global $wpdb;

        $wpdb->query("UPDATE {$wpdb->prefix}pco_signups SET event_name = REPLACE(event_name, '\\\\\\'', '\\'')");
        $wpdb->query("UPDATE {$wpdb->prefix}pco_signups SET payment_description = REPLACE(payment_description, '\\\\\\'', '\\'')");
        $wpdb->query("UPDATE {$wpdb->prefix}pco_registrations SET first_name = REPLACE(first_name, '\\\\\\'', '\\'')");
        $wpdb->query("UPDATE {$wpdb->prefix}pco_registrations SET last_name = REPLACE(last_name, '\\\\\\'', '\\'')");

        update_option('pco_slashes_fixed', true);

        add_action('admin_notices', function() {
            echo '<div class="notice notice-success"><p>Slashes removed from database!</p></div>';
        });
    }

    /**
     * Set the webhook handler for Google Forms integration
     */
    public function set_webhook_handler($webhook_handler) {
    ...
    public function add_admin_menu_page() {
        ...
        add_submenu_page(
                'pco-services',
                __('Form Registrations', 'pco-aio'),
                __('Registrations', 'pco-aio'),
                'edit_posts',
                'pco-registrations',
    ...
    public function settings_init() {
        ...
        // Add these lines at the end of settings_init()
        register_setting('pco_settings_group', 'stripe_secret_key');
        register_setting('pco_settings_group', 'stripe_webhook_secret');

        add_settings_section(
                'stripe_section',
                'Stripe Payment Settings',
                function() {
                    echo '<p>Configure Stripe. Get keys from <a href="https://dashboard.stripe.com/apikeys" target="_blank">Stripe Dashboard</a>.</p>';
                },
                'pco-settings'
        );
        ...
        add_settings_field(
                'stripe_secret_key',
                'Secret Key',
                [$this, 'stripe_secret_key_callback'],
                'pco-settings',
                'stripe_section'
        );
        ...
        add_settings_field(
                ...
                'Webhook Secret',
                [$this, 'stripe_webhook_secret_callback'],
                'pco-settings',
                'stripe_section'
        );
    }
    ...
    /**
     * Render Registration Detail View
     */
    private function render_registration_detail($submission_id) {
        $submission = $this->webhook_handler->get_submission($submission_id);
        ...
        $responses = json_decode($submission->form_data, true);
        ?>
        <div class="wrap">
            <h1 class="wp-heading-inline">Submission Details</h1>
            <a href="<?php echo admin_url('admin.php?page=pco-registrations'); ?>" class="page-title-action">← Back to List</a>
            <hr class="wp-header-end">
            ...
            <div class="card" style="margin-top: 20px; padding: 20px; max-width: 800px;">
                <table class="form-table">
                    <tr>
                        ...
                        <td><strong>#<?php echo intval($submission->id); ?></strong></td>
                    </tr>
                    <tr>
                        <th>Form:</th>
                        <td><strong><?php echo esc_html($submission->form_title); ?></strong></td>
                    </tr>
                    <tr>
                        <th>Submitted:</th>
                        <td><?php echo esc_html(mysql2date('l, F j, Y \a\t g:i A', $submission->submitted_at)); ?></td>
                    </tr>
                    <?php if ($submission->submitter_email): ?>
                        <tr>
                            <th>Email:</th>
                            <td><a href="mailto:<?php echo esc_attr($submission->submitter_email); ?>"><?php echo esc_html($submission->submitter_email); ?></a></td>
                        </tr>
                    <?php endif; ?>
                </table>
                ...
                <table class="widefat striped">
                    ...
                    <?php foreach ($responses as $question => $answer): ?>
                        <tr>
                            <td><strong><?php echo esc_html($question); ?></strong></td>
                            <td><?php echo nl2br(esc_html(is_array($answer) ? implode(', ', $answer) : $answer)); ?></td>
                        </tr>
                    <?php endforeach; ?>
                    </tbody>
                </table>
                ...
                <p style="margin-top: 30px;">
                    <a href="<?php echo wp_nonce_url(admin_url('admin.php?page=pco-registrations&action=delete&id=' . $submission->id), 'delete_submission_' . $submission->id); ?>" class="button button-secondary" onclick="return confirm('Delete this submission?');">Delete Submission</a>
                </p>
            </div>
        </div>
        <?php
...
// Additional methods for pco-admin.php
// Add these methods to your existing pco_admin class

    /**
     * SIGNUPS MANAGEMENT PAGES
     */

    /**
     * Main Signups Page - Router
     */
    ...
    private function render_signups_list_page() {
        ...
        if (isset($_GET['action']) && $_GET['action'] === 'delete' && isset($_GET['id'])) {
            ...
            echo '<div class="notice notice-success is-dismissible"><p>Signup deleted successfully.</p></div>';
        }

        $signups = $this->signups_handler->get_signups();
        ?>
        <div class="wrap">
            <h1 class="wp-heading-inline">Event Signups</h1>
            <a href="<?php echo admin_url('admin.php?page=pco-signups&view=new'); ?>" class="page-title-action">Add New</a>
            <hr class="wp-header-end">

            <?php if (empty($signups)): ?>
                <div class="notice notice-info"><p>No signups created yet. Click "Add New" to create your first signup.</p></div>
            <?php else: ?>
                <table class="wp-list-table widefat fixed striped">
                    <thead>
                    <tr>
                    ...
                    <?php foreach ($signups as $signup):
                        $reg_count = $this->signups_handler->get_registration_count($signup->id, false);
                        $capacity_text = $signup->max_attendees > 0 ? "{$reg_count} / {$signup->max_attendees}" : $reg_count;
                        $payment_text = $signup->payment_required ? '$' . number_format($signup->payment_amount, 2) : '—';
                        $status_class = $signup->is_active ? 'success' : 'error';
                        $status_text = $signup->is_active ? 'Active' : 'Inactive';
                        ?>
                        <tr>
                            <td><?php echo intval($signup->id); ?></td>
                            <td>
                                <strong>
                                    <a href="<?php echo admin_url('admin.php?page=pco-signups&view=edit&id=' . $signup->id); ?>">
                            ...
                            <td><span class="notice notice-<?php echo $status_class; ?> inline" style="padding: 5px 10px; margin: 0;"><?php echo $status_text; ?></span></td>
                            <td>
                                <a href="<?php echo admin_url('admin.php?page=pco-signups&view=edit&id=' . $signup->id); ?>" class="button button-small">Edit</a>
                                <?php if ($signup->google_form_url): ?>
                                    <a href="<?php echo esc_url($signup->google_form_url); ?>" class="button button-small" target="_blank">View Form</a>
                                <?php endif; ?>
                                <a href="<?php echo wp_nonce_url(admin_url('admin.php?page=pco-signups&action=delete&id=' . $signup->id), 'delete_signup_' . $signup->id); ?>" class="button button-small" onclick="return confirm('Delete this signup and all its registrations?');">Delete</a>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                    </tbody>
...
// UPDATED admin pages - FIXES FOR:
// 1. Headers already sent (form processing at top)
// 2. Form ID extraction for /d/e/ pattern
// 3. Partial payment support

    /**
     * Signup Edit/Create Page - FIXED VERSION
     */
    private function render_signup_edit_page() {
        ...
        ?>
        <div class="wrap">
            <h1><?php echo $is_new ? 'Add New Signup' : 'Edit Signup'; ?></h1>
            <a href="<?php echo admin_url('admin.php?page=pco-signups'); ?>" class="page-title-action">← Back to List</a>
            <hr class="wp-header-end">

            <?php if (isset($_GET['updated'])): ?>
                <div class="notice notice-success is-dismissible"><p>Signup saved successfully!</p></div>
            <?php endif; ?>
            ...
            <form method="POST" style="max-width: 800px;">
                ...
                <table class="form-table">
                    <tr>
                        <th><label for="pco_event">Select PCO Event *</label></th>
                        <td>
                            <select name="pco_event" id="pco_event" style="width: 400px;" onchange="populateEventData(this)">
                                ...
                                <?php if (!isset($pco_events['error'])): ?>
                                    <?php foreach ($pco_events as $event):
                                        $selected = ($signup && $signup->event_id === $event['event_id']) ? 'selected' : '';
                            ...
                            <p class="description">Select an event from Planning Center Calendar</p>
                        </td>
                    </tr>
                    ...
                    <tr>
                        ...
                        <td>
                            <input type="text" name="event_name" id="event_name" style="width: 400px;" value="<?php echo esc_attr($signup->event_name ?? ''); ?>" required />
                            <input type="hidden" name="event_id" id="event_id" value="<?php echo esc_attr($signup->event_id ?? ''); ?>" />
                        </td>
                    </tr>
                    ...
                    <tr>
                        ...
                        <td>
                            <input type="datetime-local" name="event_date" id="event_date" style="width: 300px;" value="<?php echo esc_attr($signup && $signup->event_date ? date('Y-m-d\TH:i', strtotime($signup->event_date)) : ''); ?>" required />
                        </td>
                    </tr>

                    <tr>
                        <th><label for="google_form_url">Google Form URL</label></th>
                        <td>
                            <input type="url" name="google_form_url" id="google_form_url" style="width: 500px;" value="<?php echo esc_attr($signup->google_form_url ?? ''); ?>" placeholder="https://docs.google.com/forms/..." />
                            <p class="description">Paste the full URL of your Google Form</p>
                        </td>
                    </tr>

                    <tr>
                        <th><label for="google_form_id">Google Form ID</label></th>
                        <td>
                            <input type="text" name="google_form_id" id="google_form_id" style="width: 400px;" value="<?php echo esc_attr($signup->google_form_id ?? ''); ?>" readonly />
                            <p class="description">The form ID (extracted from URL automatically)</p>
                        </td>
                    </tr>

                    <tr>
                        <th><label for="max_attendees">Max Attendees</label></th>
                        <td>
                            <input type="number" name="max_attendees" id="max_attendees" class="small-text" value="<?php echo esc_attr($signup->max_attendees ?? 0); ?>" min="0" />
                            <p class="description">0 = unlimited</p>
                        </td>
                    </tr>

                    <tr>
                        <th><label for="payment_required">Payment Required</label></th>
                        <td>
                            <label>
                                <input type="checkbox" name="payment_required" id="payment_required" value="1" <?php checked($signup->payment_required ?? 0, 1); ?> onchange="togglePaymentFields(this)" />
                                Require payment for this signup
                            </label>
                        </td>
                    </tr>

                    <tr id="payment_fields" style="<?php echo ($signup && $signup->payment_required) ? '' : 'display:none;'; ?>">
                        <th><label for="payment_amount">Payment Settings</label></th>
                        <td>
                            <div style="margin-bottom: 20px;">
                                <label for="payment_amount" style="display: block; margin-bottom: 5px; font-weight: 600;">Total Payment Amount *</label>
                                <input type="number" name="payment_amount" id="payment_amount" class="regular-text" value="<?php echo esc_attr($signup->payment_amount ?? 0); ?>" min="0" step="0.01" />
                                <p class="description">Full amount required (in USD)</p>
                            </div>

                            <div style="margin-bottom: 20px; padding: 15px; background: #f0f0f0; border-radius: 4px;">
                                <label>
                                    <input type="checkbox" name="allow_partial_payment" id="allow_partial_payment" value="1" <?php checked($signup->allow_partial_payment ?? 0, 1); ?> onchange="togglePartialPayment(this)" />
                                    <strong>Allow partial/deposit payments</strong>
                                </label>

                                <div id="partial_payment_fields" style="margin-top: 15px; <?php echo ($signup && $signup->allow_partial_payment) ? '' : 'display:none;'; ?>">
                                    <label for="minimum_payment" style="display: block; margin-bottom: 5px; font-weight: 600;">Minimum Payment Amount</label>
                                    <input type="number" name="minimum_payment" id="minimum_payment" class="regular-text" value="<?php echo esc_attr($signup->minimum_payment ?? 0); ?>" min="0" step="0.01" />
                                    <p class="description">The minimum amount that must be paid now (e.g., deposit)</p>
                                </div>
                            </div>

                            <div>
                                <label for="payment_description" style="display: block; margin-bottom: 5px; font-weight: 600;">Payment Description</label>
                                ...
                                <p class="description">This will appear on the payment form and email</p>
                            </div>
                        </td>
                    </tr>
                    ...
                    <tr>
                        ...
                        <td>
                            <label>
                                <input type="checkbox" name="is_active" id="is_active" value="1" <?php checked($signup->is_active ?? 1, 1); ?> />
                ...
                <p class="submit">
                    <input type="submit" name="save_signup" class="button button-primary button-large" value="<?php echo $is_new ? 'Create Signup' : 'Update Signup'; ?>" />
                </p>
            </form>
            ...
            <script>
                ...
                // FIX #2: Auto-extract form ID from URL - FIXED REGEX for /d/e/ pattern
    ...
    /**
     * ENHANCED REGISTRATIONS PAGES
     */

    /**
     * Enhanced Registrations Page - Router
     */
    ...
    /**
     * Show signups that have registrations
     */
    private function render_signups_with_registrations_page() {
        ...
        <div class="wrap">
            ...
            <?php if (empty($signups)): ?>
                <div class="notice notice-info"><p>No registrations yet. <a href="<?php echo admin_url('admin.php?page=pco-signups'); ?>">Create a signup</a> to get started.</p></div>
            <?php else: ?>
                <table class="wp-list-table widefat fixed striped">
                    <thead>
                    <tr>
                        ...
                        <th style="width: 100px;">Waitlist</th>
                    ...
                    <?php foreach ($signups as $signup):
                        ...
                        <tr>
                            ...
                            <td>
                                <strong>
                                    <a href="<?php echo admin_url('admin.php?page=pco-registrations&view=registrants&signup_id=' . $signup->id); ?>">
                                        <?php echo esc_html($signup->event_name); ?>
                                    </a>
                                </strong>
                            </td>
                            <td><?php echo esc_html(mysql2date('M j, Y', $signup->event_date)); ?></td>
                            <td><strong><?php echo intval($signup->confirmed_count) . $capacity_display; ?></strong></td>
                            <td><?php echo esc_html($waitlist_display); ?></td>
                            <td>
                                <a href="<?php echo admin_url('admin.php?page=pco-registrations&view=registrants&signup_id=' . $signup->id); ?>" class="button button-small">View Registrants</a>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                    </tbody>
    ...
    /**
     * Show registrants for a specific signup (with color coding)
     */
    private function render_registrants_list_page($signup_id) {
        ...
        if (!$signup) {
            echo '<div class="wrap"><div class="notice notice-error"><p>Signup not found.</p></div></div>';
            return;
        }

        $registrations = $this->signups_handler->get_registrations($signup_id);
        $confirmed_count = 0;
        foreach ($registrations as $reg) {
            if (!$reg->is_waitlist) $confirmed_count++;
        }

        ?>
        <div class="wrap">
            <h1 class="wp-heading-inline"><?php echo esc_html($signup->event_name); ?> - Registrants</h1>
            <a href="<?php echo admin_url('admin.php?page=pco-registrations&view=add&signup_id=' . $signup_id); ?>" class="page-title-action">New registration</a>
            <a href="<?php echo admin_url('admin.php?page=pco-registrations'); ?>" class="page-title-action">← Back</a>
            <hr class="wp-header-end">

            <div class="card" style="padding: 20px; margin: 20px 0;">
                <p style="margin: 0; font-size: 16px;">
                    <strong>Date:</strong> <?php echo esc_html(mysql2date('l, F j, Y \a\t g:i A', $signup->event_date)); ?><br>
                    <strong>Total Registered:</strong> <?php echo $confirmed_count; ?> <?php echo $signup->max_attendees > 0 ? "/ {$signup->max_attendees}" : ""; ?><br>
                    <?php if ($signup->payment_required): ?>
                        <strong>Payment:</strong> $<?php echo number_format($signup->payment_amount, 2); ?>
                    <?php endif; ?>
                </p>
            </div>

            <?php if (empty($registrations)): ?>
                <div class="notice notice-info"><p>No registrations yet.</p></div>
            <?php else: ?>
                <table class="wp-list-table widefat fixed striped">
                    <thead>
                    <tr>
                    ...
                    <?php foreach ($registrations as $idx => $reg):
                        // Color coding
                        $has_balance = $signup->payment_required && ($reg->payment_status !== 'paid');
                        $color = $has_balance ? '#f0ad4e' : '#5cb85c';
                        $balance_text = $has_balance ? '$' . number_format($reg->payment_amount - $reg->amount_paid, 2) : '—';
                        $status_text = $reg->is_waitlist ? 'Waitlist' : 'Confirmed';
                        $full_name = trim($reg->first_name . ' ' . $reg->last_name) ?: 'No name';
                        ?>
                        <tr>
                            <td style="background-color: <?php echo $color; ?>; padding: 0;"></td>
                            <td><?php echo $idx + 1; ?></td>
                            <td>
                                <strong>
                                    <a href="<?php echo admin_url('admin.php?page=pco-registrations&view=detail&id=' . $reg->id); ?>">
                                        <?php echo esc_html($full_name); ?>
                                    </a>
                                </strong>
                                <?php if ($reg->email): ?>
                                    <br><small><?php echo esc_html($reg->email); ?></small>
                                <?php endif; ?>
                            </td>
                            <td><?php echo esc_html(mysql2date('M j, Y g:i A', $reg->registration_date)); ?></td>
                            <?php if ($signup->payment_required): ?>
                                <td><?php echo esc_html($balance_text); ?></td>
                            <?php endif; ?>
                            <td><?php echo esc_html($status_text); ?></td>
                            <td>
                                <a href="<?php echo admin_url('admin.php?page=pco-registrations&view=detail&id=' . $reg->id); ?>" class="button button-small">View</a>
                                <a href="<?php echo wp_nonce_url(admin_url('admin.php?page=pco-registrations&view=registrants&signup_id=' . $signup_id . '&action=delete&reg_id=' . $reg->id), 'delete_registration_' . $reg->id); ?>" class="button button-small" onclick="return confirm('Delete this registration?');">Delete</a>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                    </tbody>
    ...
    /**
     * Show individual registrant details
     */
    private function render_registrant_detail_page($registration_id) {
        ...
        if (!$registration) {
            echo '<div class="wrap"><div class="notice notice-error"><p>Registration not found.</p></div></div>';
            return;
        }

        $signup = $this->signups_handler->get_signup($registration->signup_id);
        $form_data = json_decode($registration->form_data, true);
        $full_name = trim($registration->first_name . ' ' . $registration->last_name) ?: 'No name';
        ?>
        <div class="wrap">
            <h1 class="wp-heading-inline"><?php echo esc_html($full_name); ?></h1>
            <a href="<?php echo admin_url('admin.php?page=pco-registrations&view=registrants&signup_id=' . $registration->signup_id); ?>" class="page-title-action">← Back to Registrants</a>
            <hr class="wp-header-end">
            ...
            <div class="card" style="margin-top: 20px; padding: 20px; max-width: 800px;">
                <table class="form-table">
                    ...
                    <tr>
                        <th>Email:</th>
                        <td><?php echo $registration->email ? '<a href="mailto:' . esc_attr($registration->email) . '">' . esc_html($registration->email) . '</a>' : '—'; ?></td>
                    </tr>
                    <tr>
                        <th>Phone:</th>
                    ...
                    <tr>
                        ...
                        <td><?php echo esc_html(mysql2date('l, F j, Y \a\t g:i A', $registration->registration_date)); ?></td>
                    </tr>
                    <tr>
                        <th>Status:</th>
                        <td><?php echo $registration->is_waitlist ? '<span style="color: #e67e22;">Waitlist</span>' : '<span style="color: #27ae60;">Confirmed</span>'; ?></td>
                    </tr>
                    <?php if ($signup->payment_required): ?>
                        <tr>
                            ...
                            <td>
                                <?php
                                $status_colors = ['pending' => '#f0ad4e', 'paid' => '#5cb85c', 'partial' => '#5bc0de'];
                                $status_color = $status_colors[$registration->payment_status] ?? '#999';
                                ?>
                                <span style="color: <?php echo $status_color; ?>; font-weight: bold; text-transform: uppercase;">
                        ...
                        <?php if ($registration->payment_amount > $registration->amount_paid): ?>
                            <tr>
                                <th>Balance:</th>
                                <td style="color: #e74c3c; font-weight: bold;">$<?php echo number_format($registration->payment_amount - $registration->amount_paid, 2); ?></td>
                            </tr>
                        <?php endif; ?>
                    <?php endif; ?>
                </table>
    ...
    /**
     * Manual registration add page - UPDATED with payment total display
     */
    private function render_add_registration_page($signup_id) {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_registration'])) {
            check_admin_referer('add_registration');

            // Check capacity
            $current_count = $this->signups_handler->get_registration_count($signup_id, false);
            $is_waitlist = ($signup->max_attendees > 0 && $current_count >= $signup->max_attendees) ? 1 : 0;

            $amount_paid = floatval($_POST['amount_paid'] ?? 0);
            $payment_amount = $signup->payment_required ? $signup->payment_amount : 0;

            // Determine payment status
            if (!$signup->payment_required) {
                $payment_status = 'paid';
            ...
            $data = [
                    ...
                    'form_data' => ['Manual Entry' => 'Added by admin'],
            ...
            $this->signups_handler->save_registration($data);

            wp_redirect(admin_url('admin.php?page=pco-registrations&view=registrants&signup_id=' . $signup_id . '&added=1'));
            exit;
        }

        $show_partial = $signup->allow_partial_payment ?? 0;
        $min_payment = $signup->minimum_payment ?? 0;
        ?>
        <div class="wrap">
            <h1>Add Manual Registration</h1>
            <a href="<?php echo admin_url('admin.php?page=pco-registrations&view=registrants&signup_id=' . $signup_id); ?>" class="page-title-action">← Back</a>
            <hr class="wp-header-end">

            <div class="card" style="padding: 15px; margin: 20px 0; background: #e8f4f8;">
                <p style="margin: 0;">
                    <strong>Event:</strong> <?php echo esc_html($signup->event_name); ?> |
                    <strong>Date:</strong> <?php echo esc_html(mysql2date('M j, Y', $signup->event_date)); ?>
                    <?php if ($signup->payment_required): ?>
                        | <strong>Payment Total:</strong> $<?php echo number_format($signup->payment_amount, 2); ?>
                    <?php endif; ?>
                </p>
            </div>
            ...
            <form method="POST" style="max-width: 600px;">
                ...
                <table class="form-table">
                    ...
                    <?php if ($signup->payment_required): ?>
                        <tr>
                            <th><label for="amount_paid">Payment Information</label></th>
                            <td>
                                <div style="background: #f9f9f9; padding: 15px; border-radius: 4px; margin-bottom: 15px;">
                                    <p style="margin: 0 0 10px 0; font-size: 16px;">
                                        <strong>Total Amount:</strong>
                                        <span style="font-size: 20px; color: #2271b1;">$<?php echo number_format($signup->payment_amount, 2); ?></span>
                                    </p>

                                    <?php if ($show_partial): ?>
                                        <p style="margin: 0; font-size: 14px; color: #666;">
                                            Minimum payment: $<?php echo number_format($min_payment, 2); ?>
                                        </p>
                                    <?php endif; ?>
                                </div>

                                <label for="amount_paid" style="display: block; margin-bottom: 5px; font-weight: 600;">
                                    Amount Paid Now
                                </label>
                                <input type="number" name="amount_paid" id="amount_paid" class="regular-text" value="0" min="0" step="0.01" />

                                <?php if ($show_partial): ?>
                                    <p class="description">
                                        Enter the amount being paid now. Must be at least $<?php echo number_format($min_payment, 2); ?>
                                    </p>
                                <?php else: ?>
                                    <p class="description">
                                        Enter $<?php echo number_format($signup->payment_amount, 2); ?> if paying in full, or $0 if payment will be made later
                                    </p>
                                <?php endif; ?>
                            </td>
                        </tr>
                ...
                <p class="submit">
                    <input type="submit" name="add_registration" class="button button-primary button-large" value="Add Registration" />
                </p>
            </form>
        </div>
        <?php
    ...
    public function stripe_secret_key_callback() {
        ...
        echo '<p class="description">Your Stripe Secret Key (starts with sk_)</p>';
    }

    public function stripe_webhook_secret_callback() {
        $value = get_option('stripe_webhook_secret');
        $webhook_url = rest_url('pco-stripe/v1/webhook');
        echo '<input type="text" name="stripe_webhook_secret" value="' . esc_attr($value) . '" class="regular-text" />';
        echo '<p class="description">Webhook signing secret from Stripe Dashboard</p>';
        echo '<p class="description"><strong>Webhook URL:</strong> <code>' . esc_html($webhook_url) . '</code></p>';
    }
}