...
class MyPCO_Credentials_Manager {
    ...
    /**
     * Store Clearstream credentials (encrypted).
     */
    public static function set_clearstream_credentials($api_token, $message_header = '') {
        update_option(self::CLEARSTREAM_API_TOKEN, self::encrypt($api_token));
        update_option(self::CLEARSTREAM_MESSAGE_HEADER, sanitize_text_field($message_header));
    }
    
    /**
     * Get Clearstream credentials (decrypted).
     */
    public static function get_clearstream_credentials() {
        $encrypted_token = get_option(self::CLEARSTREAM_API_TOKEN);
        
        return [
            'api_token' => self::decrypt($encrypted_token),
            'message_header' => get_option(self::CLEARSTREAM_MESSAGE_HEADER, '')
        ];
    }
